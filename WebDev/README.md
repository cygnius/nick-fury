...
* **Implementation**: Once the API design and the database design is finalized and reviewed, you will start with the implementation. The Java APIs need to be deployable as a AWS API Gateway + Lambda solution as a AWS CDK application. You should be testing your APIs through Postman. Make a [Postman collection](https://www.postman.com/collection/) containing all your API calls. All functionality should be executable over these APIs. Important Points:
    ...
    * If you don't know about AWS, backend API, serverless paradigm of computing, you can learn it here:
        ...
        * Udemy Course - [Serverless using AWS Lambda for Java Developers](https://www.udemy.com/course/serverless-programming-using-aws-lambda-for-java-developers/). This course teaches you multiple thing - Serverless, Lambda function application code, Lambda functions infrastructure (through SAM), API Gateway, S3, etc. This is to build your foundation of how things come together in the AWS ecosystem. Please note that AWS SAM is an old & error-prone way of expressing the infrastructure. AWS has learnt from these mistakes and created CDK for expressing & managing the infrastructure which is less error-prone and much more maintainable. We will be using CDK to create our assignment so please dont spend too much time understanding SAM. Udemy Credentials at the bottom of the page. Please note that other students are also using this account, so the Udemy app will not be able to remember which chapter/lecture you were at in the previous learning session. Please note down the lecture number and play time in the lecture when you finish your learning session.
